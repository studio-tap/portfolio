...
## 2025-10-19 (Part 4): Vercelでの独自ドメイン・課金設定

- **目的**:
  - Vercelで独自ドメインを適用し、Proプランにおける意図しない従量課金を防ぐための設定を行う。

- **ドメイン設定の学習**:
  - **ネームサーバーの変更**: Vercelで独自ドメインを利用する推奨方法は、ドメインレジストラ（お名前.com）側で、ネームサーバーをVercelが指定するものに変更する方式であることを確認。
  - **外部メールサーバーとの連携**: ネームサーバーをVercelに変更するとDNS管理がVercelに移管されるため、外部のメールサーバー（さくらインターネット）を継続利用するには、VercelのDNS設定画面でMXレコードとTXT(SPF)レコードを手動で再設定する必要があることを理解した。
  - **Aレコードの扱い**: Webサイト用のAレコードはVercelが自動で最適に設定するため、ユーザーが手動で設定する必要はないことを確認。

- **課金設定 (Proプラン) の学習と実践**:
  - **Spend Management**: Proプランにおける意図しない従量課金を防ぐには、`Billing`設定にある`Spend Management`機能が不可欠であることを学習。
  - **On-demand Budgetの設定**:
